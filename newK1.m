...
num_absorbers = 20;  % Set the number of absorbers to zero
absorber_frequency_shift = 0.02; % Small shift in stiffness to slightly change frequency
absorber_mass = 6/num_absorbers;  % Assigning a small mass to each absorber
...
for i = 1:num_absorbers
    ...
    new_column = [0; -absorber_stiffness; 0; zeros(N+i-4, 1)];
    K = [K,new_column];  % Add corresponding stiffness
    % K
    new_row = [0,-absorber_stiffness,0,zeros(1, N+i-4),absorber_stiffness];
    new_row
    K = [K; new_row];
    K
end
