...
num_absorbers = 100;  % Set the number of absorbers to zero
absorber_frequency_shift = 0.01; % Small shift in stiffness to slightly change frequency
absorber_mass = 3/num_absorbers;  % Assigning a small mass to each absorber
...
for i = 1:num_absorbers
    ...
    new_column = zeros(N+i-1, 1);
    new_column(end) = - absorber_stiffness; 
    K = [K,new_column];  % Add corresponding stiffness
    % K
    new_row = [zeros(1, N+i-2),-absorber_stiffness,absorber_stiffness];
    new_row
    K = [K; new_row];
    % K
    K(N+i-1,N+i-1)=K(N+i-1,N+i-1)+absorber_stiffness;
    % K
end
